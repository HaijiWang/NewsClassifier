...
public class SolrConnection {
	...
	public void resetQueryParams() {
		//System.out.println("Debugz: IN resetQUeryParams");
	...
	public void setFieldById(String id, String filedName,String value){
		
		System.out.println("Debug: Going to update id:"+ id);
		...
		update_params.put(filedName,fieldModifier);
		update_params.put("id",id);
		// Somehow it does not have to be parsed, what can I say !!!!!
		//update_params.put("id",QueryParser.escape(id)); 
		try{
			this.updateContent(update_params);
		} catch(Exception e){
			System.out.println("Error: Failed to update id:"+ id);
			System.out.println(e);
		}
		System.out.println("Successful! Finished updating id:"+ id);
	}
	
	public SolrDocument getFieldById(String id, ArrayList<String> fieldNames){
		
		this.resetQueryParams();
		query_params.put("q", "id:" + QueryParser.escape(id));
		query_params.put("start","0");
		query_params.put("rows","1");
		Iterator<String> itr = fieldNames.iterator();
		String field_str = "";
		while(itr.hasNext()){
			field_str = field_str+itr.next()+ ",";
		}
		field_str = field_str+"id";
		query_params.put("fl",field_str);
		
		QueryResponse solr_response = new QueryResponse();
		SolrDocument doc = new  SolrDocument();
		
		try {
			solr_response = this.getContent();	
			doc = solr_response.getResults().get(0);
			//System.out.println("Degbug: solr_response: "+solr_response.toString());
		} catch(Exception e){
			
			System.out.println(e);
			System.out.println("Error in SolrConnection.getFieldById)().");
		}
		return doc;
	}
